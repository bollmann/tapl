...
applyRecord :: Term -> Term
applyRecord r = (Lam "r" (RcdT ["x"] [NatT]) $ (Var "r") `Proj` "x") `App` r
...
-- FIXME: checking prop_factorial is *awfully* slow. How can we make
-- it faster?
...
typeTests = TestList
  ...
  , typeOf (applyRecord (Rcd ["x", "y"] [toNat 1, toNat 2])) ~?= Right NatT
  , isLeft (typeOf (applyRecord (Rcd ["x"] [Lam "x" NatT (Var "x")])))
      ~? "argument record has wrong type"
  ]
